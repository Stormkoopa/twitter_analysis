...
"""
Twitter Topic Analysis with Coherence Score

Workflow:
1. Fetch tweets using Twitter API v2 (Recent Search).
2. Preprocess text: cleaning, tokenization, stopword removal.
3. Apply intelligent filtering: n-grams (bi-/trigrams) and TF-IDF percentile filtering.
4. Extract entities: most frequent hashtags and most active users.
5. Perform topic modeling with LDA (Latent Dirichlet Allocation).
6. Use coherence score (c_v) to determine the optimal number of topics.
7. Save results as CSV files for further inspection.

Outputs:
- tweets_raw.csv          → raw tweets with metadata
- tweets_clean.csv        → cleaned tweets
- hashtags_top.csv        → most frequent hashtags
- users_top.csv           → most active users
- topics_top.csv          → top terms per topic
- coherence_scores.csv    → coherence values for different topic numbers
"""
...
def clean_text(s: str) -> str:
    """
    Clean tweet text by removing URLs, mentions, hashtags, 
    ...
    s = s.lower()
    s = NON_WORD_RE.sub(" ", s)   # keep only letters/numbers
    s = WS_RE.sub(" ", s).strip()
    return s
...
def tokenize_basic(s: str) -> List[str]:
    ...
    - remove stopwords, numbers, and very short tokens
    """
...
def build_phrases(token_lists: List[List[str]], min_count=8, threshold=8.0) -> List[List[str]]:
    ...
    Returns a new token list with combined n-grams.
    """
    if not token_lists:
        return token_lists
...
def fetch_tweets(client, query: str, max_tweets: int = 400) -> Tuple[pd.DataFrame, dict]:
    ...
    Returns a DataFrame with tweet data and a user map.
    """
    rows, user_map = [], {}
...
def extract_entities(df: pd.DataFrame, user_map: dict):
    ...
    for _, r in df.iterrows():
        for h in re.findall(r"#(\w+)", r.get("text", "")):
            hashtags[h.lower()] += 1
        aid = r.get("author_id")
        if pd.notna(aid):
            users[aid] += 1
...
def pick_best_lda(token_lists: List[List[str]], k_grid=range(3, 11), passes=8, random_state=42):
    ...
    Returns the best model and a list of all tested coherence scores.
    """
...
def main():
    ...
    parser.add_argument("--max_tweets", type=int, default=400)
    ...
    if args.reuse and os.path.exists(os.path.join(args.out_dir, "tweets_raw.csv")):
        ...
        print(f"[INFO] Query: {args.query} | max_tweets={args.max_tweets}")
